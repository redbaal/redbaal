...
Server_redbaal = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
------------------------------------------------------------------------------------------------------------
local function Load_File()
local f = io.open("./Info_Sudo.lua", "r")  
if not f then   
if not redis:get(Server_redbaal.."Token_Devredbaal") then
...
redis:set(Server_redbaal.."Token_Devredbaal",token)
end 
else
io.write('\n\27[1;31m⌔︙Token was not saved \n لم يتم حفظ التوكن \n\27[0;39;49m')
end 
os.execute('lua redbaal.lua')
end
------------------------------------------------------------------------------------------------------------
if not redis:get(Server_redbaal.."User_Devredbaal1") then
io.write('\n\27[1;35m⌔︙Send ID For Sudo : ارسل ايدي المطور الاساسي ...\n\27[0;39;49m')
local User_Sudo = io.read():gsub('@','')
if User_Sudo ~= '' then
io.write('\n\27[1;31m⌔︙The ID Is Saved : تم حفظ ايدي المطور\n\27[0;39;49m')
redis:set(Server_redbaal.."Id_Devredbaal",User_Sudo)
io.write('\n\27[1;35m⌔︙Send UserName For Sudo : ارسل معرف المطور الاساسي ...\n\27[0;39;49m')
local User_Sudo2 = io.read():gsub('@','')
if User_Sudo ~= '' then
redis:set(Server_redbaal.."User_Devredbaal1",User_Sudo2)
end
else
...
------------------------------------------------------------------------------------------------------------
local Devredbaal_Info_Sudo = io.open("Info_Sudo.lua", 'w')
Devredbaal_Info_Sudo:write([[
do 
local redbaal_INFO = {
Id_Devredbaal = ]]..redis:get(Server_redbaal.."Id_Devredbaal")..[[,
UserName_redbaal = "]]..redis:get(Server_redbaal.."User_Devredbaal1")..[[",
Token_Bot = "]]..redis:get(Server_redbaal.."Token_Devredbaal")..[["
}
return redbaal_INFO
end

]])
Devredbaal_Info_Sudo:close()
------------------------------------------------------------------------------------------------------------
local Run_File_redbaal = io.open("redbaal", 'w')
Run_File_redbaal:write([[
#!/usr/bin/env bash
cd $HOME/redbaal
token="]]..redis:get(Server_redbaal.."Token_Devredbaal")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./redbaal.lua -p PROFILE --bot=$token
done
]])
Run_File_redbaal:close()
------------------------------------------------------------------------------------------------------------
...
while(true) do
rm -fr ../.telegram-cli
screen -S redbaal -X kill
screen -S redbaal ./redbaal
done
]])
Run_SM:close()
io.popen("mkdir Files")
os.execute('chmod +x tg')
os.execute('chmod +x redbaal')
os.execute('chmod +x tk')
os.execute('./tk')
Status = true
else   
f:close()  
redis:del(Server_redbaal.."Token_Devredbaal");redis:del(Server_redbaal.."Id_Devredbaal");redis:del(Server_redbaal.."User_Devredbaal1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_redbaal
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_Devredbaal
Ids_Dev = {sudos.Id_Devredbaal,bot_id}
Name_Bot = redis:get(bot_id.."Redis:Name:Bot") or "ريدبول"
------------------------------------------------------------------------------------------------------------
function var(value)  
...
------------------------------------------------------------------------------------------------------------
function Dev_redbaal(msg)  
local Dev_redbaal = false  
...
function Dev_redbaal_User(user)  
local Dev_redbaal_User = false  
for k,v in pairs(Ids_Dev) do  
if user == v then  
Dev_redbaal_User = true  
end  
end  
return Dev_redbaal_User  
end 
function DeveloperBot(msg)  
local Status = redis:sismember(bot_id.."Developer:Bot", msg.sender_user_id_) 
if Status or Dev_redbaal(msg) or Bot(msg) then  
return true  
else  
return false  
end  
...
if hash or Dev_redbaal(msg) or DeveloperBot(msg) or Bot(msg) then  
return true 
else 
return false 
end 
...
if hash or Dev_redbaal(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_redbaal(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_redbaal(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_redbaal(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_redbaal(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Admin(msg) or Bot(msg) then     
return true 
else 
return false 
end 
...
if Dev_redbaal_User(user_id) then
Status = true  
...
if Dev_redbaal_User(user_id) == true then
...
url = 'https://raw.githubusercontent.com/redbaal/redbaal/main/redbaal.lua'
file_path = 'redbaal.lua'
local respbody = {} 
...
data.first_name_ = gmatch or 'redbaal'
end
...
------------------------------------------------------------------------------------------------------------
function Filesredbaal(msg)
File_Bot = dofile("redbaal.lua")
if File_Bot.redbaal and msg then
Text_File = File_Bot.redbaal(msg)
end
send(msg.chat_id_, msg.id_,Text_File)  
return false
end
function FilesredbaalBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.redbaalFile and msg then
Text_FileBot = Text_FileBot.redbaalFile(msg)
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_redbaal_File(msg,data)
if msg then
msg = data.message_
...
send(msg.chat_id_, msg.id_,'\n⌔︙الكانسر مقفول يرجى زغرفه اسمك اولاً\n ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉  ┉ ┉ ┉ ┉\n[⌔︙ اضغط هنا لزغرفه اسمك.](https://t.me/NE_SR1BOT)')
https.request("https://api.telegram.org/bot"..token.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id="..msg.sender_user_id_)
end
end,nil)   
return false
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'GetTexting:Devredbaal'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'GetTexting:Devredbaal'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'⌔︙تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Texting:Devredbaal',text)
redis:del(bot_id..'GetTexting:Devredbaal'..msg.chat_id_..':'..msg.sender_user_id_)
...
if Dev_redbaal(msg) then
if text == 'نقل الاحصائيات' then
local Users = redis:smembers(bot_id.."redbaal:UsersBot")
local Groups = redis:smembers(bot_id..'redbaal:Chek:Groups') 
for i = 1, #Groups do
redis:sadd(bot_id..'ChekBotAdd',Groups[i])  
local list1 = redis:smembers(bot_id..'redbaal:Basic:Constructor'..Groups[i])
for k,v in pairs(list1) do
redis:sadd(bot_id.."President:User"..Groups[i], v)
end
local list2 = redis:smembers(bot_id..'redbaal:Constructor'..Groups[i])
for k,v in pairs(list2) do
redis:sadd(bot_id.."Constructor:Group"..Groups[i], v)
end
local list3 = redis:smembers(bot_id..'redbaal:Manager'..Groups[i])
for k,v in pairs(list3) do
redis:sadd(bot_id.."Manager:Group"..Groups[i], v)
end
local list4 = redis:smembers(bot_id..'redbaal:Mod:User'..Groups[i])
...
download_to_file('https://raw.githubusercontent.com/redbaal/redbaal/main/redbaal.lua','redbaal.lua') 
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/redbaalTeam/Files_redbaal/master/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
if Get_info then
local TextS = "\n⌔︙قائمه ملفات متجر سورس redbaal\n⌔︙الملفات المتوفره حاليا\n━━━━━━━━━━━━━\n\n"
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/redbaalTeam/Files_redbaal/master/Files_redbaal/"..File_Name)
if Res == 200 then
...
if not Dev_redbaal(msg) and not redis:sismember(bot_id..'User:Ban:Pv',msg.sender_user_id_) and not redis:get(bot_id..'Status:Lock:Twasl') then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_, msg.id_,'- هلو حب ، لايمكنك استخدام البوت 💕.\n- ﭑشترك اولاً 💕 • ['..redis:get(bot_id..'add:ch:username')..'] .')
return false
end
...
redis:set(bot_id..'GetTexting:Devredbaal'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
redis:del(bot_id..'Texting:Devredbaal')
...
if Dev_redbaal_User(result.id_) == true then
...
send(msg.chat_id_, msg.id_,'*⌔︙ عدد المجموعات الان ← { '..#group..' } مجموعه '..redbaal..''..taha..'\n⌔︙اصبح عدد المجموعات الان ← { '..sendok..' } مجموعات*\n')   
end
end
end,nil)
end
...
if text == 'رفع النسخه الاحتياطيه' and tonumber(msg.reply_to_message_id_) > 0 and Dev_redbaal(msg) then   
tdcli_function({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},function(Arg, Data)   
...
SetFile_Groups(msg,msg.chat_id_,Data.content_.document_.document_.persistent_id_ ,Data.content_.document_.file_name_)
            ...
            local planther = math.random(100)
            if text == ("نسبه الحب") and tonumber(msg.reply_to_message_id_) > 0 then
                send(msg.chat_id_, msg.id_, "⌔︙نسبه الحب بينكم : " .. planther .. "%")
                return false
            end
            if text == "نسبه الكره" and tonumber(msg.reply_to_message_id_) > 0 then
                send(msg.chat_id_, msg.id_, "⌔︙نسبه الكره بينكم : " .. planther .. "%")
                return false
            end
            if text == "نسبه رجولته" and tonumber(msg.reply_to_message_id_) > 0 then
                send(msg.chat_id_, msg.id_, "⌔︙نسبه رجولته : " .. planther .. "%")
                return false
            end
            if text == "نسبه انوثته" and tonumber(msg.reply_to_message_id_) > 0 then
                send(msg.chat_id_, msg.id_, "⌔︙نسبه انوثته : " .. planther .. "%")
                return false
            end
            if text == "نسبه غبائه" and tonumber(msg.reply_to_message_id_) > 0 then
                send(msg.chat_id_, msg.id_, "⌔︙نسبه غبائه : " .. planther .. "%")
                return false
            end
            if text == "هينه" and tonumber(msg.reply_to_message_id_) > 0 then
                local planther = {"يقواد كعد وسكت ،", "زباله شحجي وياك",
                                  "طفل شحجي وياك"}
                send(msg.chat_id_, msg.reply_to_message_id_, planther[math.random(#planther)])
                return false
            end
            if text == "هينها" and tonumber(msg.reply_to_message_id_) > 0 then
                local planther = {"ادبسز متكعدين راحه ،", "زباله شحجي وياج",
                                  "زعطوطه شحجي وياج"}
                send(msg.chat_id_, msg.reply_to_message_id_, planther[math.random(#planther)])
                return false
            end
            if text == "بوسها" or text == "بوسه" and tonumber(msg.reply_to_message_id_) > 0 then
                local planther = {"مح ،", "محححهههه 😂😭💞. ", "محححح 🌚🖤."}
                send(msg.chat_id_, msg.reply_to_message_id_, planther[math.random(#planther)])
                return false
            end
            if text and text:match("^نسبه الحب (.*) و (.*)$") then
                send(msg.chat_id_, msg.id_, "⌔︙نسبه الحب بينهم : " .. planther .. "%")
                return false
            end
            if text and text:match("^نسبه الكره (.*) و (.*)$") then
                send(msg.chat_id_, msg.id_, "⌔︙نسبه الكره بينهم : " .. planther .. "%")
                return false
            end
            if text and text:match("^نسبه رجوله (.*)$") then
                send(msg.chat_id_, msg.id_, "⌔︙نسبه رجولته : " .. planther .. "%")
                return false
            end
            if text and text:match("^كشف الكذب (.*)$") then
                local tex = text:match("^كشف الكذب (.*)$")
                local planther = {" احس هل شي كذب 🌚💕", " فعلا كل الي كلته صح 😻💋",
                                  "صح %100", " صحيحة",
                                  " كذب كذب كذب كذب شلون بلا اصيح كذب 🌚😹👌",
                                  " ڪلُآمك صادق 💘"}
                send(msg.chat_id_, msg.id_,
                    "الجمـلة ..💛 (" .. tex .. ")\n" .. planther[math.random(#planther)])
                return false
            end
            if text and text:match("^نسبه انوثته (.*)$") then
                send(msg.chat_id_, msg.id_, "⌔︙نسبه انوثته : " .. planther .. "%")
                return false
            end
            if text and text:match("^نسبه غباء (.*)$") then
                send(msg.chat_id_, msg.id_, "⌔︙نسبه غبائه : " .. planther .. "%")
                return false
            end
            
            if text == "مسح الصخول" and Admin(msg) then
                redis:del(bot_id .. 'z:bot' .. msg.chat_id_)
                send(msg.chat_id_, msg.id_, "⌔︙لايربك تم 😂😭")
                return false
            end
            if text == ("تاك للصخول") and Admin(msg) then
                local list = redis:smembers(bot_id .. "z:bot")
                planther = "\n ⌔︙لايربك 😂😭\n\n"
                for k, v in pairs(list) do
                    local username = redis:get(bot_id .. "Save:Username" .. v)
                    if username then
                        planther = planther .. "" .. k .. " - الصخول : [@" .. username .. "]\n"
                    else
                        planther = planther .. "" .. k .. " - الصخول : `" .. v .. "`\n"
                    end
                end
                if #list == 0 then
                    planther = "- ماكو صخول"
                end
                send(msg.chat_id_, msg.id_, planther)
            end
            ---------
            if text == ("رفع صخل") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
                if AddChannel(msg.sender_user_id_) == false then
                    send(msg.chat_id_, msg.id_,
                        '\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- '.. redis:get(bot_id..'add:ch:username')..'')
                    return false
                ...
                function FunctionStatus(arg, result)
                    redis:sadd(bot_id .. "z:bot", result.sender_user_id_)
                    planther = '- ماكو داعي ارفعه صخل هوه صخل من كاعه'
                    send(msg.chat_id_, msg.id_, planther)
                    send(msg.chat_id_, msg.reply_to_message_id_, "- وكف جيبلك لعلف")
                end
                tdcli_function({
                    ID = "GetMessage",
            ...
            if text == ("تنزيل صخول") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
                function FunctionStatus(arg, result)
                    redis:srem(bot_id .. "z:bot", result.sender_user_id_)
                    planther = '- تم بس لصخل يبقه صخل'
                    send(msg.chat_id_, msg.id_, planther)
                end
                tdcli_function({
                    ID = "GetMessage",
            ...
            if text == "مسح المطايه" and Admin(msg) then
                redis:del(bot_id .. 'mote:bot' .. msg.chat_id_)
                send(msg.chat_id_, msg.id_, "⌔︙لايربك تم 😂😭")
                return false
            end
            if text == ("تاك للمطايه") and Admin(msg) then
                local list = redis:smembers(bot_id .. "mote:bot")
                planther = "\n ⌔︙لايربك 😂😭\n\n"
                for k, v in pairs(list) do
                    local username = redis:get(bot_id .. "Save:Username" .. v)
                    if username then
                        planther = planther .. "" .. k .. " - المطي : [@" .. username .. "]\n"
                    else
                        planther = planther .. "" .. k .. " - المطي : `" .. v .. "`\n"
                    end
                end
                if #list == 0 then
                    planther = "- ماكو مطايه"
            ...
            ---------
            if text == ("رفع مطي") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
                if AddChannel(msg.sender_user_id_) == false then
                    send(msg.chat_id_, msg.id_,
                        '\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- '.. redis:get(bot_id..'add:ch:username')..'')
                    return false
                ...
                function FunctionStatus(arg, result)
                    redis:sadd(bot_id .. "mote:bot", result.sender_user_id_)
                    planther = '- ماكو داعي ارفعه مطي هوه مطي من كاعه'
                    send(msg.chat_id_, msg.id_, planther)
                    send(msg.chat_id_, msg.reply_to_message_id_, "- تع ستلم لعربانه")
                end
                tdcli_function({
                    ID = "GetMessage",
                    chat_id_ = msg.chat_id_,
            ...
            if text == ("تنزيل مطي") and tonumber(msg.reply_to_message_id_) ~= 0 and Admin(msg) then
                function FunctionStatus(arg, result)
                    redis:srem(bot_id .. "mote:bot", result.sender_user_id_)
                    planther = '- تم بس لمطي يبقه مطي'
                    send(msg.chat_id_, msg.id_, planther)
                    send(msg.chat_id_, msg.reply_to_message_id_, "- رجع لعربانه تع")
                end
                tdcli_function({
                    ID = "GetMessage",
                    chat_id_ = msg.chat_id_,
            ...
            if text == ("تاك لنسواني") then
                local list = redis:smembers(bot_id .. "mygril"..msg.sender_user_id_..":bot")
                planther = "\n ⌔︙نسوانك 😂😭\n\n"
                for k, v in pairs(list) do
                    local username = redis:get(bot_id .. "Save:Username" .. v)
                    if username then
                        planther = planther .. "" .. k .. " - مرتك : [@" .. username .. "]\n"
                    else
                        planther = planther .. "" .. k .. " - مرتك : `" .. v .. "`\n"
                    end
                end
                if #list == 0 then
                    planther = "- ياهي تقبل بيك انت"
                end
                send(msg.chat_id_, msg.id_, planther)
            end
            ---------
            if text == ("رفع مرتي") and tonumber(msg.reply_to_message_id_) ~= 0 then
                if AddChannel(msg.sender_user_id_) == false then
                    send(msg.chat_id_, msg.id_,
                        '\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- '.. redis:get(bot_id..'add:ch:username')..'')
                    return false
                ...
                function FunctionStatus(arg, result)
                    redis:sadd(bot_id .. "mygril"..msg.sender_user_id_..":bot", result.sender_user_id_)
                    planther = '- تم بس لمهر شكد'
                    send(msg.chat_id_, msg.id_, planther)
                    send(msg.chat_id_, msg.reply_to_message_id_, "- هوه صح اجكم بس شنسوي مو زين اكو احد قبل بيج")
                end
                tdcli_function({
                    ID = "GetMessage",
                    chat_id_ = msg.chat_id_,
            ...
            if text == ("تنزيل مرتي") and tonumber(msg.reply_to_message_id_) ~= 0 then
                function FunctionStatus(arg, result)
                    redis:srem(bot_id .. "mygril"..msg.sender_user_id_..":bot", result.sender_user_id_)
                    planther = '- تم 😂😭'
                    send(msg.chat_id_, msg.id_, planther)
                    send(msg.chat_id_, msg.reply_to_message_id_, "- طالك طالك طالك")
                end
                tdcli_function({
                    ID = "GetMessage",
                    chat_id_ = msg.chat_id_,
...
if text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if text and text:match("^اضف مطور @(.*)$") and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if text and text:match("^حذف مطور @(.*)$") and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if text and text:match("^اضف مطور (%d+)$") and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if text and text:match("^حذف مطور (%d+)$") and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if text == 'جلب نسخه احتياطيه' and Dev_redbaal(msg) or text == 'جلب نسخه الكروبات' and Dev_redbaal(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd')  
...
if text and text:match("^تغير الاشتراك$") and Dev_redbaal(msg) then  
redis:setex(bot_id.."add:ch:jm" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 360, true)  
send(msg.chat_id_, msg.id_, '܁༯┆حسنا ارسل المعرف الان 💞 ܰ') 
return false  
end
if text and text:match("^وضع قناة الاشتراك$") and Dev_redbaal(msg) then  
redis:setex(bot_id.."add:ch:jm" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 360, true)  
send(msg.chat_id_, msg.id_, '܁༯┆ارسل معرف قناتك 💞 ܰ') 
return false  
end
if text == "تفعيل الاشتراك" and Dev_redbaal(msg) then  
if redis:get(bot_id..'add:ch:id') then
...
if text == ("مسح قائمه العام") and Dev_redbaal(msg) or text == ("مسح المحظورين عام") and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
redis:del(bot_id.."Removal:User:Groups")
send(msg.chat_id_, msg.id_, "⌔︙تم مسح المحظورين عام من البوت")
elseif text == ("مسح المطورين") and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
elseif text == ("قائمه العام") and Dev_redbaal(msg) or text == ("المحظورين عام") and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
send(msg.chat_id_, msg.id_, Gban)
elseif text == ("المطورين") and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
elseif text == ("حظر عام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
if Dev_redbaal_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text == ("الغاء العام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text and text:match("^حظر عام @(.*)$") and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حظر عام @(.*)$")}, FunctionStatus, nil)
elseif text and text:match("^الغاء العام @(.*)$") and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
elseif text and text:match("^حظر عام (%d+)$") and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
if Dev_redbaal_User(text:match("^حظر عام (%d+)$")) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
elseif text and text:match("^الغاء العام (%d+)$") and Dev_redbaal(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- ['..redis:get(bot_id..'add:ch:username')..']')   
return false
end
...
elseif text == "تفعيل المغادره" and Dev_redbaal(msg) then   
redis:del(bot_id.."Status:Lock:Left"..msg.chat_id_)  
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل مغادرة البوت") 
elseif text == "تعطيل المغادره" and Dev_redbaal(msg) then  
redis:set(bot_id.."Status:Lock:Left"..msg.chat_id_,true)   
send(msg.chat_id_, msg.id_, "⌔︙تم تعطيل مغادرة البوت") 
elseif text == "تفعيل الاذاعه" and Dev_redbaal(msg) then  
redis:del(bot_id.."Status:Broadcasting:Bot") 
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل الاذاعه \n⌔︙الان يمكن للمطورين الاذاعه" ) 
elseif text == "تعطيل الاذاعه" and Dev_redbaal(msg) then  
redis:set(bot_id.."Status:Broadcasting:Bot",true) 
send(msg.chat_id_, msg.id_,"⌔︙تم تعطيل الاذاعه") 
...
elseif text == 'تفعيل البوت الخدمي' and Dev_redbaal(msg) then  
redis:del(bot_id..'Free:Bot') 
send(msg.chat_id_, msg.id_,'⌔︙تم تفعيل البوت الخدمي \n⌔︙الان يمكن الجميع تفعيله') 
elseif text == 'تعطيل البوت الخدمي' and Dev_redbaal(msg) then  
redis:set(bot_id..'Free:Bot',true) 
send(msg.chat_id_, msg.id_,'⌔︙تم تعطيل البوت الخدمي') 
...
elseif text == 'وضع كليشه المطور' and Dev_redbaal(msg) then
redis:set(bot_id..'GetTexting:Devredbaal'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_,msg.id_,'⌔︙ ارسل لي الكليشه الان')
...
elseif text == 'روابط الكروبات' and Dev_redbaal(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd') 
test = '⌔︙روابط الكروبات \n\n'
for k,v in pairs(list) do 
...
elseif text == ("مسح ردود المطور") and Dev_redbaal(msg) then 
...
elseif text == ("ردود المطور") and Dev_redbaal(msg) then 
local list = redis:smembers(bot_id.."List:Rd:Sudo")
text = "\n⌔︙قائمة ردود المطور \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
...
send(msg.chat_id_, msg.id_,"["..text.."]")
elseif text == "اضف رد للكل" and Dev_redbaal(msg) then 
...
elseif text == "حذف رد للكل" and Dev_redbaal(msg) then 
...
if Dev_redbaal_User(msg.sender_user_id_) == true then
redis:srem(bot_id.."Developer:Bot", result.id_)
...
elseif text == "تغير اسم البوت" and Dev_redbaal(msg) or text == "تغيير اسم البوت" and Dev_redbaal(msg) then 
redis:setex(bot_id.."Change:Name:Bot"..msg.sender_user_id_,300,true) 
send(msg.chat_id_, msg.id_,"⌔︙ ارسل لي الاسم الان ")  
elseif text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and DeveloperBot(msg) then 
if redis:get(bot_id.."Status:Broadcasting:Bot") and not Dev_redbaal(msg) then 
...
𓂅 .السورس خاص
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
𓂅 . [Source Channel](t.me/R6CCC)

 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 

𓂅 . [مجموعة قنواتنا ](t.me/mx34xm)     
 
 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
𓂅 . [TWS redbaal](t.me/gh_gh90)    
]]) 
...
⌔︙ ارسل { اوامر التحشيش } ← لعرض اوامر التحشيش┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
⌔︙قناة البوت ←* [@R6CCC]
]]) 
...
*]])
elseif text == 'اوامر التحشيش' and Admin(msg) then
send(msg.chat_id_, msg.id_,[[*
⌔︙اوامر التحشيش اتبع مايلي ...
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
⌔︙ تفعيل اوامر التحشيش 
⌔︙ تعطيل اوامر التحشيش 
⌔︙┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},reply, nil)
return false
elseif text == "تنظيف المشتركين" and Dev_redbaal(msg) then
local pv = redis:smembers(bot_id..'Num:User:Pv')  
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]},function(arg,dataq)
...
return false
elseif text == "تنظيف الكروبات" and Dev_redbaal(msg) then
local group = redis:smembers(bot_id..'ChekBotAdd')  
local w = 0
local q = 0
for i = 1, #group do
...
elseif text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_redbaal(msg) then
redis:set(bot_id..'Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
send(msg.chat_id_, msg.id_,'*⌔︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : '..text:match("تعين عدد الاعضاء (%d+)$")..' عضو *')
elseif text =='الاحصائيات' and DeveloperBot(msg) then 
send(msg.chat_id_, msg.id_,'*⌔︙عدد احصائيات البوت الكامله \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n⌔︙عدد المجموعات : '..(redis:scard(bot_id..'ChekBotAdd') or 0)..'\n⌔︙عدد المشتركين : '..(redis:scard(bot_id..'Num:User:Pv') or 0)..'*')
elseif text == 'المطور' or text == 'مطور' then
local TextingDevredbaal = redis:get(bot_id..'Texting:Devredbaal')
if TextingDevredbaal then 
send(msg.chat_id_, msg.id_,TextingDevredbaal)
else
send(msg.chat_id_, msg.id_,'['..UserName_Dev..']')
end
elseif text == 'حذف كليشه المطور' and Dev_redbaal(msg) then
redis:del(bot_id..'Texting:Devredbaal')
send(msg.chat_id_, msg.id_,'⌔︙ تم حذف كليشه المطور')
end
end
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Num:Add:Bot') or 0) and not Dev_redbaal(msg) then
...
--------------------------------------------------------------------------------------------------------------
Dev_redbaal_File(msg,data)
FilesredbaalBot(msg,data)
